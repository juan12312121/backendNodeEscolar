...
import * as service from './materia_grado.service';
...
export const create = asyncHandler(async (req: Request, res: Response) => {
    ...
    return sendSuccess(res, record, 'Relacion materia-grado creada', undefined, 201);
});

export const update = asyncHandler(async (req: Request, res: Response) => {
    const record = await service.update(req.params.id, req.body as UpdateInput);
    return sendSuccess(res, record, 'Relacion materia-grado actualizada');
});

export const remove = asyncHandler(async (req: Request, res: Response) => {
    await service.remove(req.params.id);
    return sendSuccess(res, null, 'Relacion materia-grado eliminada');
});
