...
import { CreateInput, UpdateInput } from './materias.validation';
...
export async function getAll(query: QueryParams & { id_plan?: string }): Promise<PaginatedResult<any>> {
    ...
    const [data, total] = await Promise.all([
        prisma.materias.findMany({ where, skip, take: limit, orderBy: { f_cre: 'desc' } }),
...
export async function getById(id: string) {
    const record = await prisma.materias.findUnique({ where: { id } });
    if (!record) throw new NotFoundError('Materia no encontrada');
    return record;
}

export async function create(data: CreateInput) {
    return prisma.materias.create({ data });
}

export async function update(id: string, data: UpdateInput) {
    await getById(id);
    return prisma.materias.update({ where: { id }, data });
}

export async function remove(id: string) {
    await getById(id);
    return prisma.materias.update({ where: { id }, data: { activo: false } });
}
